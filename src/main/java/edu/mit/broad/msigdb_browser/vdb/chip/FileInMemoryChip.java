...
public class FileInMemoryChip extends AbstractChip {
    ...
    private void initHere(final String chipName, final String sourcePath, final Probe[] probes) {
        ...
        if (NamingConventions.isURL(sourcePath) == false) {
            ...
            if (sourcePath == null) {
                missing = true;
            } else if (new File(sourcePath).exists() == false) {
                missing = true;
            } else if (sourcePath.equals(COMBO_DUMMY_SOURCE) == false) {
                missing = true;
            }
    ...
    private void readDeepData() throws Exception {
        ...
        try {
            ...
            FileInMemoryChip chip = (FileInMemoryChip) ParserFactory.readChip(fSourcePath, true);
